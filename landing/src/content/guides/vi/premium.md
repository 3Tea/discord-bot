---
title: Premium
description: Nâng cấp trải nghiệm với gói Star và Galaxy — đọc manga nhiều hơn, cooldown nhanh hơn, và các đặc quyền độc quyền.
icon: "👑"
order: 3
relatedCommands: ["premium", "wallet"]
---

## Tổng Quan

3AT cung cấp hai gói premium — **Star** ⭐ và **Galaxy** 🌌 — nâng cấp trải nghiệm của bạn trên tất cả server. Premium là **theo người dùng và toàn cầu**: mua một lần, dùng mọi nơi.

## So Sánh Gói

| Quyền lợi | Miễn phí | ⭐ Star | 🌌 Galaxy |
|-----------|----------|---------|-----------|
| Lượt manga miễn phí/ngày | 3 | 10 | Không giới hạn |
| Số trang manga tối đa | 35 | 70 | 100 |
| Thời gian chờ làm việc | 4h | 2h | 1h |
| Thời gian chờ câu cá | 1h | 30m | 15m |
| Thời gian chờ khai thác | 2h | 1h | 30m |
| Thời gian chờ hầm ngục | 1h | 30m | 15m |
| Bonus tỉ lệ rơi star | — | +50% | +100% |
| Bỏ qua CD confession | 50 coin | Miễn phí | Miễn phí |
| Confession VIP | 5 gem | 5 gem | Miễn phí |
| Star bonus hàng ngày | — | — | +2 |
| Huy hiệu rank card | — | ⭐ Star | 🌌 Galaxy |
| Theme rank card | Tiêu chuẩn | Tiêu chuẩn | Galaxy độc quyền |

> **Lưu ý:** Người dùng miễn phí có thể sử dụng mọi tính năng. Premium giúp mọi thứ nhanh hơn, nhiều hơn, và đẹp hơn — không bao giờ khóa nội dung.

> **⚠️ Tính năng manga hiện chưa ổn định** và chưa được hỗ trợ đầy đủ. Các quyền lợi liên quan đến manga (lượt miễn phí, số trang tối đa) có thể chưa hoạt động đúng. Vui lòng cân nhắc trước khi mua premium chủ yếu vì tính năng manga.

## Bảng Giá

### Việt Nam

| Gói | Hàng tháng | Hàng năm | Tiết kiệm |
|-----|-----------|----------|-----------|
| ⭐ Star | 29,000 VND | 290,000 VND | ~17% |
| 🌌 Galaxy | 59,000 VND | 590,000 VND | ~17% |

### Quốc tế

| Gói | Hàng tháng | Hàng năm | Tiết kiệm |
|-----|-----------|----------|-----------|
| ⭐ Star | $1.99 | $19.99 | ~16% |
| 🌌 Galaxy | $3.99 | $39.99 | ~16% |

> **Gói năm** sắp ra mắt — giá hiển thị để tham khảo. Hiện tại chỉ có thanh toán hàng tháng. Bạn có thể mua bằng VND hoặc USD — cả hai đều được hỗ trợ.

## Cách Mua Premium

### Thanh toán qua Ko-fi (cho mọi user)

**Mua một lần** (kích hoạt thủ công):

| Gói | Liên kết |
|-----|----------|
| ⭐ Star | [Mua Star – Tier 1](https://ko-fi.com/s/d46298013d) |
| 🌌 Galaxy | [Mua Galaxy – Tier 2](https://ko-fi.com/s/e903c72e58) |

**Đăng ký hàng tháng** (tự động gia hạn):

Đăng ký tại [trang Ko-fi](https://ko-fi.com/3atendlessparadox) để tự động gia hạn mỗi tháng — không cần mua lại.

### Thanh toán bằng VND

> Hiện tại chưa hỗ trợ thanh toán bằng VND. Vui lòng sử dụng Ko-fi ở trên để mua premium.

### Sau Khi Mua

1. Chụp ảnh màn hình xác nhận đơn hàng
2. Tham gia [Server Hỗ Trợ](https://discord.gg/JFFCsv773E)
3. Vào kênh `#premium-support`
4. Gửi ảnh chụp và tên Discord của bạn — chúng tôi sẽ kích hoạt premium sớm nhất

Sau khi kích hoạt, premium của bạn hoạt động trên **mọi server** có 3AT.

## Quản Lý Premium

| Lệnh | Chức Năng |
|------|-----------|
| `/premium status` | Xem gói hiện tại, ngày hết hạn và quyền lợi đang hoạt động |
| `/premium compare` | So sánh chi tiết tất cả các gói |

## Mẹo

1. **Gói Star phù hợp nhất** với đa số người dùng — 10 lượt manga/ngày và cooldown giảm một nửa đáp ứng cả chơi thường lẫn tích cực.
2. **Gói Galaxy** dành cho người chơi nhiều — manga không giới hạn, cooldown nhanh nhất, +2 star bonus hàng ngày, và theme rank card Galaxy độc quyền.
3. **Premium cộng dồn với streak** — star bonus hàng ngày kết hợp với streak wallet daily để tối đa thu nhập star.
4. **Tất cả giảm cooldown có hiệu lực ngay lập tức** — ngay khi premium được kích hoạt, lệnh tiếp theo của bạn dùng cooldown đã giảm.
