...
import { approveConfession } from "../services/confession/confession.service";
import { BUTTON_ID } from "../util/config/button";
import { resolveLocale } from "../util/i18n/locale";
import { t } from "../util/i18n/t";

export default {
    id: BUTTON_ID.CONFESSION_APPROVE,
    async execute(interaction: ButtonInteraction): Promise<void> {
        ...
        const result = await approveConfession(interaction);

        if (result.ok) {
            await interaction.editReply({ content: t(locale, "confession.approve_success") });
            return;
        }
