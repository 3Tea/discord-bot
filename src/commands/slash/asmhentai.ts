...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) => subcommand.setName("random").setDescription("Random H and D from asmhentai")),
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            if (subcommand != "random") {
                asmHentai = await axios.get(`${SERVER_HD}asmhentai/get?book=${data!.options![0].value}`);
            } else {
            ...
            if (asmHentai.data?.data) {
                ...
                if (result.total < 50) {
                    ...
                    await redis.setJson(`${BUTTON_ID.asmHentaiRead}_${result.id}`, result.image, 60 * 10);
                } else {
                    row.addComponents(
                        new ButtonBuilder()
                            .setCustomId(`${BUTTON_ID.asmHentaiRead}`)
