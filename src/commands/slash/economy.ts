...
async function handleBalance(
    ...
    switch (subcommand) {
        ...
        default:
            return new EmbedBuilder().setDescription(t(locale, "common.unknown_subcommand")).setColor(0xed4245);
    }
}
...
async function handleConfig(
    ...
    switch (subcommand) {
        ...
        case "reward-toggle": {
            ...
            return new EmbedBuilder()
                .setDescription(
                    t(locale, newEnabled ? "economy.reward_config.toggled_on" : "economy.reward_config.toggled_off")
                )
        ...
        case "gambling-set": {
            ...
            await redis.deleteKey(`gambling_config:${guildId}`);
            return new EmbedBuilder().setDescription(t(locale, "gambling_config.updated")).setColor(0x57f287);
        }
        case "work-view": {
        ...
        case "work-set": {
            ...
            await redis.deleteKey(`work_config:${guildId}`);
            return new EmbedBuilder().setDescription(t(locale, "work_config.updated")).setColor(0x57f287);
        }
        case "social-view": {
        ...
        case "social-set": {
            ...
            await redis.deleteKey(`social_config:${guildId}`);
            return new EmbedBuilder().setDescription(t(locale, "social_config.updated")).setColor(0x57f287);
        }
        default:
            return new EmbedBuilder().setDescription(t(locale, "common.unknown_subcommand")).setColor(0xed4245);
    }
    ...
    switch (subcommand) {
        ...
        case "history": {
            ...
            const buildHistoryEmbed = async (
                ...
                if (result.transactions.length === 0) {
                    embed.setDescription(t(locale, "economy.admin.history.empty"));
                } else {
            ...
            while (true) {
                try {
                    ...
                    if (i.customId === "hist_next") page++;
                    else if (i.customId === "hist_prev") page = Math.max(0, page - 1);
        ...
        case "reverse": {
            ...
            try {
                ...
                if (msg === "TRANSACTION_NOT_FOUND") key = "economy.admin.reverse.not_found";
        ...
        case "reset": {
            ...
            const affectedCount = await EconomyAdminService.countAffected(guildId, scope, target);
            ...
            const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                new ButtonBuilder().setCustomId("reset_confirm").setLabel("Confirm").setStyle(ButtonStyle.Danger),
        ...
        case "rollback": {
            ...
            const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                new ButtonBuilder().setCustomId("rb_confirm").setLabel("Confirm").setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId("rb_cancel").setLabel("Cancel").setStyle(ButtonStyle.Secondary)
            );
...
async function handleBulk(
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("bulk_confirm").setLabel("Confirm").setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId("bulk_cancel").setLabel("Cancel").setStyle(ButtonStyle.Secondary)
    );
