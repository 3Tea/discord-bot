...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) => subcommand.setName("random").setDescription("Random H and D from hentaifox")),
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            if (subcommand != "random") {
                hentaiFox = await axios.get(`${SERVER_HD}hentaifox/get?book=${data!.options![0].value}`);
            } else {
            ...
            if (hentaiFox.data?.data) {
                ...
                if (result.total < 50) {
                    ...
                    await redis.setJson(`${BUTTON_ID.hentaiFoxRead}_${result.id}`, result.image, 60 * 10);
                } else {
                    row.addComponents(
                        new ButtonBuilder()
                            .setCustomId(`${BUTTON_ID.hentaiFoxRead}`)
