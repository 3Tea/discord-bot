...
async function handleChannelBlacklist(interaction, guildId, subcommand, locale) {
    ...
    await guildXPConfig_model_1.default.updateOne({ guildId }, { $setOnInsert: { guildId } }, { upsert: true });
    let updated = null;
    if (subcommand === "add") {
        updated = await guildXPConfig_model_1.default.findOneAndUpdate({ guildId, blacklistedChannels: { $ne: channel.id } }, { $addToSet: { blacklistedChannels: channel.id } }, { returnDocument: "after" });
        if (!updated) {
        ...
        updated = await guildXPConfig_model_1.default.findOneAndUpdate({ guildId, blacklistedChannels: channel.id }, { $pull: { blacklistedChannels: channel.id } }, { returnDocument: "after" });
        if (!updated) {
    ...
    const list = channels.length > 0 ? channels.map((id) => `<#${id}>`).join(", ") : (0, t_1.t)(locale, "xp.blacklist.empty");
    const embed = new discord_js_1.EmbedBuilder()
