...
exports.startVoiceXPWorker = startVoiceXPWorker;
exports.stopVoiceXPWorker = stopVoiceXPWorker;
...
async function tickVoiceXPSessions() {
    const sessions = await redis_1.default.getSetMembers(VOICE_XP_SET);
    ...
    for (const session of sessions) {
        try {
            ...
            const config = await guildXPConfig_model_1.default.findOneAndUpdate({ guildId: sGuildId }, { $setOnInsert: { guildId: sGuildId } }, { upsert: true, returnDocument: "after" });
            if (!config.enabled)
                continue;
            if (config.blacklistedChannels.includes(sChannelId))
                continue;
            ...
            if (newLevel > updated.level) {
                ...
                try {
                    ...
                    if (notifConfig.enabled && notifConfig.channelId) {
                        const notifGuild = client_1.default.guilds.cache.get(sGuildId);
                        if (notifGuild) {
                            ...
                            if (user) {
                                ...
                                await (0, notificationService_1.sendNotification)(notifGuild, notifConfig.channelId, embed, guildNotificationConfig_model_1.NotificationType.LevelUp);
                            }
                        }
                    }
                }
