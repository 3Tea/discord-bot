...
async function addGold(userId, amount) {
    ...
    const result = await character_model_1.default.findOneAndUpdate({ userId }, update, { returnDocument: "after" });
    if (!result)
        throw new CharacterNotFoundError(userId);
    await redis_1.default.deleteKey(`rpg_char:${userId}`);
...
async function deductGold(userId, amount) {
    ...
    const result = await character_model_1.default.findOneAndUpdate({ userId, gold: { $gte: amount } }, { $inc: { gold: -amount } }, { returnDocument: "after" });
    if (!result)
        throw new InsufficientGoldError(char.gold, amount);
    await redis_1.default.deleteKey(`rpg_char:${userId}`);
    return result.gold;
...
async function addMaterials(userId, materials) {
    ...
    const update = { $inc: inc };
    await character_model_1.default.updateOne({ userId }, update);
    await redis_1.default.deleteKey(`rpg_char:${userId}`);
...
async function deleteCharacter(userId) {
    await character_model_1.default.deleteOne({ userId });
    await redis_1.default.deleteKey(`rpg_char:${userId}`);
}
