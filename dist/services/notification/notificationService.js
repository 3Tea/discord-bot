...
async function getNotificationConfig(guildId, type) {
    ...
    if (cached)
        return cached;
    const config = await guildNotificationConfig_model_1.default.findOneAndUpdate({ guildId, type }, { $setOnInsert: { guildId, type } }, { upsert: true, returnDocument: "after" });
...
async function sendNotification(guild, channelId, embed, type) {
    try {
