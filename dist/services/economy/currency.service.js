...
async function getOrCreate(userId, guildId) {
    const doc = await userEconomy_model_1.default.findOneAndUpdate({ userId, guildId }, { $setOnInsert: { userId, guildId } }, { upsert: true, returnDocument: "after" });
    return doc;
}
...
async function deduct(userId, guildId, coinAmount, gemAmount, reason, metadata = {}) {
    ...
    const updated = await userEconomy_model_1.default.findOneAndUpdate(filter, { $inc: { coin: -coinAmount, gem: -gemAmount } }, { returnDocument: "after" });
    if (!updated) {
...
async function setCoin(userId, guildId, amount) {
    ...
    const updated = await userEconomy_model_1.default.findOneAndUpdate({ userId, guildId }, { $set: { coin: amount } }, { returnDocument: "after" });
...
async function setGem(userId, guildId, amount) {
    ...
    const updated = await userEconomy_model_1.default.findOneAndUpdate({ userId, guildId }, { $set: { gem: amount } }, { returnDocument: "after" });
