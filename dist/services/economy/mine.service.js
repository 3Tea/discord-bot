...
async function mine(userId, guildId) {
    const economy = await userEconomy_model_1.default.findOneAndUpdate({ userId, guildId }, { $setOnInsert: { userId, guildId, coin: 0, gem: 0, prayStreak: 0, mineDepth: 1, mineCheckpoint: 1 } }, { upsert: true, returnDocument: "after" });
    ...
    if (Math.random() < collapseRate) {
        ...
        await userEconomy_model_1.default.updateOne({ userId, guildId }, [{ $set: { coin: { $max: [{ $subtract: ["$coin", penalty] }, 0] }, mineDepth: currentCheckpoint } }], { updatePipeline: true });
        return {
            collapsed: true,
            mineral: null,
            penalty,
