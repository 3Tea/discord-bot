...
async function getOrCreate(userId) {
    const wallet = await userWallet_model_1.default.findOneAndUpdate({ userId }, { $setOnInsert: { userId, star: 0, dailyStreak: 0, claimedMilestones: [] } }, { upsert: true, new: true });
    return wallet;
}
// --- Public API ---
...
async function deductStar(userId, amount, reason, metadata = {}) {
    ...
    const wallet = await userWallet_model_1.default.findOneAndUpdate({ userId, star: { $gte: amount } }, { $inc: { star: -amount } }, { new: true });
    if (!wallet) {
