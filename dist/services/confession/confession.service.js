...
async function reserveNextConfessionNumber(guildId) {
    const updated = await guildConfessionConfig_model_1.default.findOneAndUpdate({ guildId }, { $inc: { lastConfessionNumber: 1 } }, { new: true }).exec();
    if (!updated) {
...
async function approveConfession(interaction) {
    ...
    if (!rawId || !(0, mongoose_1.isValidObjectId)(rawId)) {
...
async function handleConfessionVote(confessionMongoId, guildId, userId, voteType) {
    if (!(0, mongoose_1.isValidObjectId)(confessionMongoId)) {
        return { ok: false, code: "invalid_id" };
    }
...
async function handleConfessionReply(params) {
    ...
    const updated = await confession_model_1.default.findByIdAndUpdate(confessionMongoId, { $inc: { replyCount: 1 } }, { new: true }).exec();
    if (!updated) {
        return { ok: false, code: "not_found" };
    }
